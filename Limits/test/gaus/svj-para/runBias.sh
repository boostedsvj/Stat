...
if [[ "$region" == "highCut" ]]; then
  ...
  exit 1
	...
	echo "region: $region"
	combine -M FitDiagnostics SVJ_mZprime3000_mDark20_rinv03_alphapeak_${region}_2018_template_bias2.txt $EXTRA -t 300 --toysFrequentist --saveToys --expectSignal 0.0 --rMin -80 --rMax 80 -n bias_${ENAME}${region} $PARS -v 3
done
