...
fit_template = "{fitname}\ts:fn[{fnname}]\tvd:pars[1,{pvals}]\ts:legname[{legname}]\tin:input[input/input_svj_mt_fit_opt.txt]\tb:legpars[0]\tc:linecolor[{fitcol}]"
...
region_info = {
    ...
    "bsvj": {"alt": 3, "main": 2, "legname": "bsvj"},
}
...
#def makePostfitPlot(sig, name, method, quantile, data_file, datacard_dir, obs, injected, combo, region, init_dir=True, init_ftype=None, do_plot=False):
    ch = getChannel(region)
    seedname = None
    ...
    for q in [quantile]:
	    ...
	    #bandfname = makeErrorBand(iname,ws,fitres,region,ftype)
        except TypeError:
